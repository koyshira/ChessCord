...
async function getEloRating(playerId, connection) {
  const [row] = await executeQuery(connection, 'SELECT elo FROM leaderboard WHERE user_id = ?', [playerId]);
...
module.exports = {
  calculateElo: async function (whiteId, blackId, result, winner = null) {
    ...
    try {
      ...
      const whiteNewElo = whiteElo + ELO_K_FACTOR * (scoreMultiplier - expected);
      const blackNewElo = blackElo + ELO_K_FACTOR * (scoreMultiplier - expected);
