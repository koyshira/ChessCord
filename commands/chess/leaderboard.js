...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const result = await pool.query('SELECT * FROM leaderboard ORDER BY elo DESC');
      ...
      return interaction.reply({ content: `There was an error: \`${error.message}\``, ephemeral: true });
    }
  },
};
...
function generateLeaderboardEmbed(leaderboardData) {
  ...
  const leaderboardEmbed = {
    ...
    fields: [
      {
        name: 'Top 10',
        value: topPlayers.map((user, index) => {
