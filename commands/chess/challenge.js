...
function generateChallengeEmbed(interaction, challengedUser, challengeID, color) {
  const fields = [
    { name: 'Challenger (Black)', value: `<@${interaction.user.id}>`, inline: true },
    { name: 'Challenged Player (White)', value: challengedUser ? `<@${challengedUser.id}>` : 'AI', inline: true },
  ];
...
function createButtonRow(challengeID, challengerUser, challengedUser) {
  return {
    type: 1,
    components: [
      { type: 2, style: 3, label: 'Accept', custom_id: `accept:${challengeID}:${challengerUser}` },
      { type: 2, style: 4, label: 'Reject', custom_id: `reject:${challengeID}:${challengedUser ? challengedUser.id : 'ai'}` },
    ],
  };
}
...
async function handleAiChallenge(interaction, challengeID) {
  ...
  saveChallenge({ id: challengeID, challenger: interaction.client.user.id, challenged: interaction.user.id, lastPlayer: interaction.client.user.id, status: 'AIGame', opponentType: 'ai' });
...
async function handlePlayerChallenge(interaction, challengedUser, challengeID) {
  const embedData = generateChallengeEmbed(interaction, challengedUser, challengeID, SUCCESS_Color);

  const challengerUser = await interaction.user.id;
  const buttonRow = createButtonRow(challengeID, challengerUser, challengedUser);
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let challenger = challengedUser ? interaction.user.id : interaction.client.user.id;
    let challenged = challengedUser ? challengedUser.id : interaction.user.id;

    saveChallenge({ id: challengeID, challenger: challenger, challenged: challenged, lastPlayer: challenger, status: 'Pending', opponentType });
  },
