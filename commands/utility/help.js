...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const message = 'Chess allows you to play chess with your friends or against a bot.\nTo get started, challenge a player or bot with `/challenge (@player)`'

    const chessCommands = {
      color: 0x007aff,
      title: 'Chess Commands',
      fields: [
        { name: 'Challenge a player or bot', value: '`/challenge (@player)`', inline: true },
        { name: 'Accept a challenge', value: '`/accept (challenge_id)`', inline: true },
        { name: 'Reject a challenge', value: '`/reject (challenge_id)`', inline: true },
    ...
    const utilityCommands = {
      ...
      fields: [
        ...
        { name: 'Get the bot\'s changelog', value: '`/changelog`', inline: true },
        { name: 'Invite the bot to your server', value: '`/invite`', inline: false },
      ],
    };

    await interaction.reply({ content: message, embeds: [chessCommands, utilityCommands] });
  }
};
